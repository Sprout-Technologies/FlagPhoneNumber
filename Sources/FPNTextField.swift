...
open class FPNTextField: UITextField {
    ...
    @objc open var flagButtonSize: CGSize = .init(width: 32, height: 32) {
        didSet {
            layoutIfNeeded()
        }
    }
    ...
    private var phoneCodeTextField: UITextField = .init()

    private lazy var phoneUtil: NBPhoneNumberUtil = .init(metadataHelper: NBMetadataHelper())
    private var nbPhoneNumber: NBPhoneNumber?
    private var formatter: NBAsYouTypeFormatter?

    open var flagButton: UIButton = .init()
    ...
    open lazy var pickerView: FPNCountryPicker = .init()
